...
if __name__ == "__main__":
    ...
    for dataset in ["A", "B"]:
        results = pd.read_pickle(
            join(RESULTS_PATH, f"param_tuning_results_dataset_{dataset}_hidden_states_truncated.pkl")
        )
    ...
    metrics = {}
    results = pd.read_pickle(
        join(RESULTS_PATH, f"out_of_sample_results_llama_hidden_state_truncated.pkl")
    )
    results = results[
    ...
    for true_target in range(2):
        ...
        plt.savefig(
            join(ANALYSIS_PATH, f"linechart_{target_type}_llama.pdf"),
            format="pdf",
            bbox_inches="tight",
            transparent=True,
        )
    ...
    df_time_scores = pd.read_pickle(join(RESULTS_PATH, "oos_time_scores_hidden_states_truncated.pkl"))
    df_time_scores = (
    ...
    plt.savefig(
        join(ANALYSIS_PATH, f"linechart_scores_over_tokens_oos_100_each_type_hidden_states_truncated.pdf"),
        format="pdf",
        bbox_inches="tight",
        transparent=True,
    )
    ...
    df_results = df_results.groupby(["model", "ctg", "dataset"]).mean(numeric_only=True)
